...
def main():
    ...
    if not all([catalogs["afps"], catalogs["generos"], catalogs["estados_civiles"]]):
        show_error_message("Datos Incompletos", "Faltan catálogos de AFP, género o estado civil.")
        st.stop()
    ...
    with st.form("solicitud_form"):
        ...
        with col1:
            submit_button = st.form_submit_button(
                "✅ Registrar Solicitud", use_container_width=True, type="primary"
            )
    ...
    if submit_button:
        ...
        with st.spinner("Registrando solicitud..."):
            try:
                ...
                st.markdown("---")
                st.info(
                    "📌 Puedes consultar el estado de tu solicitud en la sección 'Solicitudes Registradas'"
                )
