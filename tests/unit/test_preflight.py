...
def test_preflight_valid_release_passes(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    repo_root = tmp_path / "repo"
    repo_root.mkdir()
    _write_git_repo(repo_root)
    bundle, manifest = _build_release(tmp_path)
...
def test_preflight_dirty_repository_fails(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    repo_root = tmp_path / "repo"
    repo_root.mkdir()
    _write_git_repo(repo_root)
...
def test_preflight_expected_sha_mismatch_fails(
...
def test_preflight_tree_sha_mismatch_fails(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    repo_root = tmp_path / "repo"
    repo_root.mkdir()
    _write_git_repo(repo_root)
    bundle, manifest = _build_release(tmp_path)
...
def test_preflight_missing_manifest_fails(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    repo_root = tmp_path / "repo"
    repo_root.mkdir()
    _write_git_repo(repo_root)
    bundle, _manifest = _build_release(tmp_path)
    _stub_common_success(monkeypatch)
...
def test_preflight_invalid_manifest_fails(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    repo_root = tmp_path / "repo"
    repo_root.mkdir()
    _write_git_repo(repo_root)
    bundle, manifest = _build_release(tmp_path)
...
def test_preflight_mutable_image_fails(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    repo_root = tmp_path / "repo"
    repo_root.mkdir()
    _write_git_repo(repo_root)
    bundle, manifest = _build_release(tmp_path)
...
def test_preflight_app_provenance_mismatch_fails(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    repo_root = tmp_path / "repo"
    repo_root.mkdir()
    ...
    assert any(
        check["name"] == "app_source_git_sha_equals_release_git_sha" and check["status"] == "FAIL"
        for check in report["checks"]
    )


def test_preflight_missing_bundle_fails(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    repo_root = tmp_path / "repo"
    repo_root.mkdir()
    _write_git_repo(repo_root)
    _bundle, manifest = _build_release(tmp_path)
    _stub_common_success(monkeypatch)
    ...
    assert any(
        check["name"] == "bundle_exists" and check["status"] == "FAIL" for check in report["checks"]
    )


def test_preflight_bundle_tampering_fails(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    repo_root = tmp_path / "repo"
    repo_root.mkdir()
    _write_git_repo(repo_root)
    bundle, manifest = _build_release(tmp_path)
...
def test_preflight_deterministic_valid_release_passes(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    repo_root = tmp_path / "repo"
    repo_root.mkdir()
