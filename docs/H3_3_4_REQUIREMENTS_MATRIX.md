...
| REQ-B-08 | Asignados y sin asignar | Conteo con/sin asignación activa | AC-11 | LEFT JOIN `tpi.asignaciones` (activa) con `COUNT(DISTINCT id_lead)` | `tests/integration/test_executive_dashboard_repository.py::test_leads_sin_asesor_have_own_bucket`, `tests/integration/test_executive_dashboard_repository.py::test_inactive_assignments_do_not_count_and_fanout_is_safe` | Sí (AC-18) | `developer-01.json` |
...
| REQ-B-12 | Resumen por asesor | Agregación por asesor | AC-12 | `GROUP BY asesor` (JOIN `tpi.asesores` para nombre) | `tests/integration/test_executive_dashboard_per_advisor.py::test_metricas_operacionales_por_asesor_stuck_assignment_and_first_management`; `tests/integration/test_executive_dashboard_per_advisor.py::test_cartera_por_asesor_exact_total_active_and_sin_asesor` | Sí (AC-18) | `developer-01.json` |
| REQ-B-13 | Cartera total y activa por asesor | Distinción cartera total vs activa | AC-12 | Cartera activa = `estado_lead NOT IN (cerrado, perdido, no_califica, duplicado)` | `tests/integration/test_executive_dashboard_per_advisor.py::test_cartera_por_asesor_exact_total_active_and_sin_asesor`; `tests/integration/test_executive_dashboard_per_advisor.py::test_cartera_por_asesor_fanout_safe_and_dimensional_filter`; `tests/integration/test_executive_dashboard_repository.py::test_cartera_activa_excludes_closed_states` | Sí (AC-18) | `developer-01.json` |
| REQ-B-14 | Casos por estado y asesor | Matriz estado × asesor | AC-12 | `GROUP BY asesor, estado_lead` con `COUNT(DISTINCT id_lead)` | `tests/integration/test_executive_dashboard_per_advisor.py::test_casos_por_estado_y_asesor_exact_matrix_counts`; `tests/unit/test_executive_dashboard_render.py::test_state_by_advisor_matrix_is_available_without_extra_columns` | Sí (AC-18) | `developer-01.json` |
...
| REQ-B-19 | Funnel y tasas observables | Tasas honestas | AC-15 | Solo transiciones observables; pre-cutover sin historia completa no generan tasas; `COUNT(DISTINCT id_lead)` | `tests/integration/test_executive_dashboard_repository.py::test_funnel_excludes_pre_cutover_leads`, `tests/integration/test_executive_dashboard_repository.py::test_funnel_missing_cutover_yields_no_rates`, `tests/integration/test_executive_dashboard_repository.py::test_funnel_period_fully_after_cutover_computes_rates`, `tests/integration/test_executive_dashboard_repository.py::test_funnel_zero_denominator_yields_no_transitions`, `tests/integration/test_executive_dashboard_repository.py::test_funnel_preserves_unknown_states`, `tests/integration/test_executive_dashboard_repository.py::test_funnel_repeated_transition_is_counted_once`, `tests/integration/test_executive_dashboard_repository.py::test_funnel_return_to_previous_state_keeps_both_transitions`; `tests/unit/test_executive_dashboard_presentation.py::test_funnel_is_available_with_complete_coverage`, `tests/unit/test_executive_dashboard_presentation.py::test_funnel_is_withheld_without_cutover`, `tests/unit/test_executive_dashboard_presentation.py::test_funnel_is_withheld_without_a_period`, `tests/unit/test_executive_dashboard_presentation.py::test_funnel_is_withheld_when_coverage_is_partial`, `tests/unit/test_executive_dashboard_presentation.py::test_funnel_is_withheld_without_observable_transitions` | Sí (AC-18) | `developer-01.json` |
...
| REQ-B-21 | Serie temporal | Gráfico de evolución | AC-11 | Render de serie temporal (presenter/template) | `tests/unit/test_executive_dashboard_presentation.py::test_timeseries_is_none_without_observations`, `tests/unit/test_executive_dashboard_presentation.py::test_timeseries_with_a_single_observation_has_no_polyline`, `tests/unit/test_executive_dashboard_presentation.py::test_timeseries_with_all_zero_values_stays_on_the_baseline`, `tests/unit/test_executive_dashboard_presentation.py::test_timeseries_never_fabricates_points`; `tests/unit/test_executive_dashboard_render.py::test_timeseries_with_a_single_point_renders_a_marker_and_no_polyline` | Sí (AC-18) | `developer-01.json` |
...
| REQ-B-23 | Estados vacíos y de error | UX coherente sin datos/error | AC-17 | Estados vacíos y de error en template | `tests/unit/test_executive_dashboard_render.py::test_dashboard_without_data_shows_coherent_empty_states`, `tests/unit/test_executive_dashboard_render.py::test_partial_failure_reports_the_section_and_keeps_the_rest`, `tests/unit/test_executive_dashboard_render.py::test_failed_section_never_renders_as_zero`, `tests/unit/test_executive_dashboard_render.py::test_global_error_returns_a_safe_page_without_internal_details` | Sí (AC-18) | `developer-01.json` |
| REQ-B-24 | Interfaz responsive | Usabilidad multi-dispositivo | AC-17 | CSS responsive (`app/web/static/css/`) | `tests/unit/test_executive_dashboard_render.py::test_charts_are_server_rendered_without_javascript_or_cdn`, `tests/unit/test_executive_dashboard_render.py::test_every_chart_has_a_title_a_summary_and_an_accessible_table`, `tests/unit/test_executive_dashboard_render.py::test_page_has_no_required_javascript_for_filtering` (contrato de UX server-rendered); responsive validado por auditoría visual (§12.9) + smoke humano | Sí (AC-18) | `developer-01.json`; `acceptance-01.json` |
| REQ-B-25 | Coherencia con la Web UX actual | Consistencia visual/estructural | AC-17 | Mismos patrones Jinja/CSS del backoffice actual | `tests/unit/test_executive_dashboard_render.py::test_normal_render_shows_both_scopes_explicitly`, `tests/unit/test_executive_dashboard_render.py::test_filters_are_a_get_form_with_labelled_controls` (patrones Jinja/CSS del backoffice); revisión visual humana | Sí (AC-18) | `developer-01.json` |
| REQ-B-26 | Ausencia de BI externo | Sin dependencia de BI | AC-17 | Sin integración de plataforma BI externa | `tests/unit/test_executive_dashboard_render.py::test_charts_are_server_rendered_without_javascript_or_cdn`, `tests/unit/test_executive_dashboard_render.py::test_no_aggregate_data_is_embedded_in_scripts_or_comments` | — (cubierto por test) | `developer-01.json` |
...
| REQ-S-02 | 008 debe reconfirmarse libre antes de desarrollarla | Número de migración correcto | AC-2 | Verificar `008` libre en todo el historial git antes de escribir el forward/rollback | `tests/unit/test_h3_3_4_migration_scripts.py::test_008_does_not_modify_or_execute_005_006_007` (008 no modifica 005/006/007; la reconfirmación de numeración libre es verificación git del Developer) | — (cubierto por verificación) | `developer-01.json` |
...
| REQ-S-07 | Sin option-settings | Sin mutación de settings | AC-7 | Plan de deploy sin `--option-settings` (invariante `no_option_settings_in_release`) | — (verificación de guards del Harness) | — (cubierto por guard) | `candidate-01.json` (deployment_plan) |
...
| REQ-S-09 | Fallos sin eliminación automática de evidencia u objetos | Sin borrado automático | AC-8 | Ningún fallo post-deploy elimina automáticamente evidencia ni objetos | `tests/unit/test_h3_3_4_migration_scripts.py::test_forward_and_rollback_are_separate_transactional_scripts`, `tests/unit/test_h3_3_4_migration_scripts.py::test_rollback_revokes_select_and_drops_view_and_index` (borrado controlado, nunca automático) | — (cubierto por test) | `developer-01.json` |
...
| REQ-S-11 | AUTH_USERS_JSON y password_hash nunca leídos ni registrados | Invariante de secretos | AC-10, AC-17 | Ningún test/consulta/log lee `AUTH_USERS_JSON` ni `password_hash` (invariante reforzado por `secret_read_deny` y `bash.forbidden_everywhere` del guard) | `tests/unit/test_database_errors.py::test_dsn_password_is_redacted`; `tests/unit/test_database_settings.py::test_safe_summary_and_string_representation_redact_password` (secretos redactados, nunca registrados) | — (invariante de seguridad) | `developer-01.json`; `pii_log_scan` (0 coincidencias) |
| REQ-S-12 | Paridad completa de H3.3/H3.3.3 | Reutilizar patrón probado | AC-4..AC-10, AC-17, AC-18 | Mismo patrón que 007 (vista mínimo privilegio, dos aprobaciones, postflight, rollback) | `tests/unit/test_h3_3_3_timeline_presentation.py`; `tests/unit/test_h3_3_3_migration_scripts.py`; `tests/unit/test_web_app.py`; `tests/unit/test_h3_3_1_rbac_pii_assignment.py` | Smoke humano | `developer-01.json`; `verification-01.json` |
| REQ-S-13 | Cobertura de tests ≥85% | Quality gate | AC-18 | `pytest --cov=app --cov-fail-under=85` en CI | — (verificado por el job de CI: `pytest --cov=app --cov-fail-under=85`) | — (cubierto por CI) | CI verde + `developer-01.json.quality_gates` |
...
| TC-6 | Orden estable y America/Santiago | REQ-A-03, REQ-A-17 | AC-4 | `tests/unit/test_h3_3_4_timeline_presentation.py::test_state_change_timestamp_is_displayed_in_america_santiago`, `tests/unit/test_h3_3_4_timeline_presentation.py::test_deterministic_tiebreak_event_then_state_change_then_note` | `developer-01.json` |
...
| TC-8 | CEO y CTO acceden | REQ-B-02 | AC-10 | `tests/unit/test_executive_dashboard_access.py::test_ceo_can_access_dashboard`, `tests/unit/test_executive_dashboard_access.py::test_cto_can_access_dashboard` | `developer-01.json` |
| TC-9 | Otros roles y anónimos reciben 403 sin datos | REQ-B-03 | AC-10 | `tests/unit/test_executive_dashboard_access.py::test_other_authenticated_role_receives_403`, `tests/unit/test_executive_dashboard_access.py::test_anonymous_receives_403` | `developer-01.json` |
...
> Desde la ronda 2 de revisión, este gate está **automatizado y endurecido** en
> `tests/unit/test_h3_3_4_requirements_matrix_gate.py`: exige la gramática canónica de
> referencias (ruta completa `tests/unit/` o `tests/integration/` y función exacta tras `::`),
> valida que toda ruta citada existe y que toda función citada está definida, rechaza
> referencias abreviadas o sin prefijo en todo el documento, exige referencia verificable en
> filas con AC automatizados, verifica las filas humanas contra AC exclusivamente humanos,
> y además comprueba que no aparece ninguna referencia a una tarea posterior, que AC-1..AC-18
> siguen cubiertos, que los requirement IDs son únicos y completos, y que ninguna fila de
> requisito queda sin AC. Incluye 10 pruebas negativas sobre copias mutadas de la matriz.
...
(`tests/integration/test_executive_dashboard_full.py::test_full_dashboard_query_count_is_bounded_and_free_of_n_plus_1`). No se agregó ningún
...
- **F1 (major, docs)** — **corregido**: la columna "Prueba automatizada" de §4
  ...
  inexistentes (nueve archivos de test planificados que no existían en el repositorio).
  Ahora citan los archivos y funciones reales de `tests/unit/` y `tests/integration/`,
  verificados contra el repositorio. El gate programático
...
- **F3 (minor, tests 'unit' que requieren Postgres)** — **aceptado como follow-up**: los
  tests de `tests/unit/test_executive_dashboard_service.py` invocan `dashboard_read_snapshot()`
  directamente (no delegado al repositorio inyectable), por lo que requieren PostgreSQL
  (9 tests). Además, `tests/unit/test_executive_dashboard_render.py::test_nav_link_is_hidden_for_other_roles`
  y `tests/unit/test_executive_dashboard_render.py::test_hiding_the_link_is_not_the_access_control`
  hacen GET `/leads` y también requieren PostgreSQL (2 tests; 11 fallos locales sin Postgres
  en total, todos verdes en CI Linux). Sin cambio de código en esta ronda; follow-up sugerido
  por el Reviewer: que el repositorio (real o falso) posea la adquisición del snapshot para
  que la etiqueta "unit" sea fiel.
- **F4 (info, LEAD_STATE_HISTORY_CUTOVER)** — **agregado al checklist humano** (§12.10):
  ...
  (automatizada, `tests/unit/test_h3_3_4_timeline_presentation.py`) de la configuración efectiva en AWS
...
## 14. Remediación ronda 2 de revisión (review-02.json)

> Corrección de `evidence/H3.3.4/reviewer/review-02.json` (decision=REJECTED, cause=docs).
> Sin cambios funcionales; sin tocar migraciones 005/006/007/008, AWS/RDS, `requirements/**`,
> GitPython, RBAC ni definiciones métricas.

- **F1 (major, gate burlable)** — **corregido**: todas las referencias de prueba de §3/§4/§5/§7
  quedan en gramática canónica (ruta completa desde la raíz `tests/unit/` o `tests/integration/`,
  con la función exacta citada tras `::`). Se eliminaron las referencias abreviadas y los
  archivos sin prefijo; se eliminaron las referencias a directorios de prueba no canónicos
  (REQ-S-07 pasó a verificación por guard; REQ-S-11 cita los tests unitarios de redacción de
  secretos). El gate `tests/unit/test_h3_3_4_requirements_matrix_gate.py` fue endurecido:
  inspecciona explícitamente la columna "Prueba automatizada"/"Prueba prevista", rechaza
  referencias sin prefijo, abreviadas o no canónicas en todo el documento, valida existencia
  de ruta y de función exacta, exige referencia verificable en filas con AC automatizados
  (salvo whitelist mínima documentada) y verifica las filas humanas contra AC exclusivamente
  humanos. Incluye 10 pruebas negativas sobre copias mutadas de la matriz.
- **F7 (major, sin pruebas de valores por asesor)** — **corregido**: nuevas pruebas de
  integración `tests/integration/test_executive_dashboard_per_advisor.py` con assertions
  exactas de `get_cartera_por_asesor` (cartera total/activa, exclusión exacta de
  `cerrado`/`perdido`/`no_califica`/`duplicado`, inclusión de estados activos/desconocidos,
  bucket "Sin asesor", sin fan-out), `get_casos_por_estado_y_asesor` (matriz estado×asesor
  con conteos exactos, sin fan-out, filtros) y `get_metricas_operacionales_por_asesor`
  (estancados, tiempo a asignación/primera gestión, N/D sin base, mismo universo que el
  dashboard). REQ-B-12/REQ-B-13/REQ-B-14 citan estas funciones.
- **F8 (minor, atribución del EXPLAIN)** — **corregido**: REQ-A-12 distingue el test de
  existencia del índice (`test_support_index_exists_and_is_used_for_state_history_query`)
  del test de EXPLAIN real (`test_explain_state_history_uses_support_index`); la atribución
  de AC-3 se corrige en `developer-03.json`.
- **F2/F4/F5/F6** — **preservados** según la disposición del Reviewer (F2: EXPLAIN normal sin
