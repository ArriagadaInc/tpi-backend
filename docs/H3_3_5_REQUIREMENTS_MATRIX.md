...
| 7 | Listado `/leads` scoped a cartera | AC-6, AC-7, AC-8 | `app/services/solicitud_service.py::get_crm_bandeja` + repo `portfolio_asesor_id` | `tests/unit/test_h3_3_5_advisor_service.py`, `tests/integration/test_crm_lite_board.py` | AC-17 |
| 8 | `/leads/board` scoped a cartera | AC-8 | mismo flujo `get_crm_bandeja` (ruta `/leads/board`) | `tests/unit/test_h3_3_5_advisor_service.py` | AC-17 |
| 9 | Búsqueda respeta cartera (server-side) | AC-8 | `_build_crm_query_filters` + `portfolio_asesor_id` | `tests/unit/test_h3_3_5_advisor_service.py`, `tests/integration/test_crm_lite_board.py` | AC-17 |
| 10 | Filtros (AFP, estado, fechas, origen, fuente, sin_asignar, estancado) respetan cartera | AC-8 | `_build_crm_query_filters` | `tests/integration/test_crm_lite_board.py` | AC-17 |
| 11 | Ordenamiento respeta cartera | AC-8 | `get_crm_solicitudes` (ORDER BY dentro de la misma consulta filtrada) | `tests/integration/test_crm_lite_board.py` | AC-17 |
| 12 | Conteos respetan cartera | AC-8 | `get_crm_solicitudes` (`COUNT(*)` con el mismo `where_clause`) | `tests/unit/test_h3_3_5_advisor_service.py`, `tests/integration/test_crm_lite_board.py` | AC-17 |
| 13 | Paginación respeta cartera | AC-8 | `get_crm_solicitudes` (`LIMIT/OFFSET` tras el filtro de cartera) | `tests/integration/test_crm_lite_board.py` | AC-17 |
| 14 | Detalle de lead propio | AC-6, AC-7, AC-11 | `get_solicitud_detalle_masked` (rama advisor con ownership) | `tests/unit/test_h3_3_5_advisor_service.py` | AC-17 |
| 15 | Detalle de lead ajeno / sin asignar → 404 | AC-9, AC-10 | `get_solicitud_detalle_masked` → `None`; `lead_active_assignment_belongs_to` | `tests/unit/test_h3_3_5_advisor_service.py` | AC-17 |
| 16 | Cambio de estado propio permitido / ajeno rechazado sin escribir | AC-10, AC-11 | `update_lead_status(actor=..., advisor_scope=...)` + repo ownership | `tests/unit/test_h3_3_5_advisor_service.py`, `tests/integration/test_lead_state_history.py` | AC-17 |
| 17 | Comentarios/notas propios permitidos / ajenos rechazados | AC-10, AC-11 | `append_lead_comment(actor=..., advisor_scope=...)` + repo ownership | `tests/unit/test_h3_3_5_advisor_service.py` | AC-17 |
| 18 | Cleanup según política vigente (advisor no permitido) | AC-10 | `_CLEANUP_ROLES` (sin cambios: advisor excluido → 403) | `tests/unit/test_web_app.py` | AC-17 |
| 19 | Asignación/reasignación siempre prohibida para advisor | AC-12 | `can_assign_lead` (sin cambios) + ruta `POST /leads/{id}/assign` 403 | `tests/unit/test_h3_3_5_advisor_service.py`, `tests/unit/test_web_app.py` | AC-17 |
...
| 25 | Seed DEV (Asesor Desarrollo 1/2) idempotente, no ejecutado | AC-1, AC-2, AC-18 | `scripts/sql/dev/seed_asesores_desarrollo.sql` | `tests/unit/test_h3_3_5_seed_sql.py` (verificación estática) | AC-19 (postflight read-only Deployer) |
| 26 | Preflight operacional del seed (STS, cuenta, región, env, endpoint, base, usuario, salud) | AC-18, D5 | `docs/H3_3_5_DEV_RUNBOOK.md` (fuentes canónicas) | `tests/unit/test_h3_3_5_seed_sql.py` | AC-19 |
| 27 | Registro preexistente: adoptar UUID único o abortar | AC-1, AC-2, AC-18, D6 | `seed_asesores_desarrollo.sql` (DO plpgsql por `lower(nombre)`) | `tests/unit/test_h3_3_5_seed_sql.py` | AC-19 |
| 28 | Idempotencia (2ª ejecución = mismo resultado) | AC-18, D6 | `seed_asesores_desarrollo.sql` | `tests/unit/test_h3_3_5_seed_sql.py` | AC-19 |
| 29 | Postflight DB read-only (Deployer) | AC-19, D3 | `docs/H3_3_5_DEV_RUNBOOK.md` sección postflight | — | AC-19 (Deployer) |
| 30 | Carga y refresh de `AUTH_USERS_JSON` | AC-21, D7 | evidencia de código en runbook (parseo en `SimpleDevAuth.__init__`) | `tests/unit/test_h3_3_5_advisor_auth.py` | AC-17 (refresh controlado humano) |
...
| --- | --- | --- |
| AC-1 | Asesor Desarrollo 1 único en `tpi.asesores` (crea/adopta/aborta) | seed SQL + test estático |
| AC-2 | Asesor Desarrollo 2 único en `tpi.asesores` (crea/adopta/aborta) | seed SQL + test estático |
...
| AC-6 | Asesor Desarrollo 1 ve solo su cartera + PII propia | servicio cartera + PII contextual |
| AC-7 | Asesor Desarrollo 2 ve solo su cartera + PII propia | servicio cartera + PII contextual |
| AC-8 | Búsqueda/filtros/tablero/orden/conteo/paginación server-side | `portfolio_asesor_id` en repo |
| AC-9 | Detalle ajeno/no asignado → 404 | `lead_active_assignment_belongs_to` |
| AC-10 | Mutación ajena/no asignada rechazada sin escribir (status/comments/cleanup) | ownership en servicio/repo |
| AC-11 | Acciones autorizadas solo sobre leads propios | ownership en servicio/repo |
...
| AC-16 | PII contextual por lead (no permiso global) | `get_solicitud_detalle_masked` ownership |
| AC-17 | Smoke AWS DEV autenticado (post-cutover) | runbook cutover + smoke |
| AC-18 | Seed idempotente + preflight operacional humano | seed SQL + runbook |
